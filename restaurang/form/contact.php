...
### Enter Your Sitename 
$sitename = 'Happy Order';

### Enter your email addresses: @required
$emailTO[] = array( 'email' => 'adam@happyorder.io', 'name' => 'Adam Leitzler' ); 
...
### Success Messages
$msg_success = "Vi har tagit emot ditt meddelande och hör av oss så snart som möjligt!";
...
if( $_SERVER['REQUEST_METHOD'] == 'POST') {
	if (isset($_POST["contact-email"]) && $_POST["contact-email"] != '' && isset($_POST["contact-name"]) && $_POST["contact-name"] != '') {
		...
		if ($honeypot == '' && !(empty($emailTO))) {
			...
			$bodymsg .= $_SERVER['HTTP_REFERER'] ? '<br>---<br><br>This email was sent from Happy Order: ' . $_SERVER['HTTP_REFERER'] : '';
